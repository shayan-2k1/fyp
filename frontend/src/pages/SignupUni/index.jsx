...
const Signup = () => {
  ...
  const handleChange = async (e) => {
    ...
    try {
      ...
      const r=await axios.post("http://127.0.0.1:5000/auth",
        {
          email:email,
          uniname:uniname,
        });
        if (r.data.predictions===1)
        {
          const response = await axios.post(
          "http://localhost:3000/university/registerUniversity",
          {
            email:email,
            uniname:uniname,
            password:password,
        ...
        else if(r.data.predictions===0)
        {
          alert("Email is not authorized for sign up")
        }
                    ...
                    <div className="flex flex-col font-nunito gap-2 h-[585px] md:h-auto items-start justify-start max-w-[700px] mt-[81px] w-full">
                      ...
                      <div className="flex flex-col gap-3 items-start justify-start w-full">
                        ...
                        <Input
                          ...
                          value={uniname}
                          onChange={(e) => {
                            console.log("uniname: ", e.target.value);
